vez = "BRANCO"; //variavel que controla a vez de quem jogar
lacoAux = 0; //variavel que controla o laço de iteração
andou = 0; //variavel que controla os movimentos das peças
passo = 0; //variavel que controla se o jogo vai ser passo a passou ou automatizado

//função para andar passo a passo
function andar(){
  lacoAux = 0;
  passo = 1;
  document.getElementById("automatico").setAttribute("disabled", true);
  laco();
}

//função para iniciar automatizado
function iniciar(){
  document.getElementById("passo_a_passo").setAttribute("disabled", true);
  laco();
}

//laço de repetição automatizado
function laco(){
  if(lacoAux == 0){
    ...
    //separa as peças por cor e tipo
    ...
    if(vez == 'BRANCO'){
      andou = 0;
      //busca eliminar com a dama
      if(pecas_damas_brancas.length > 0){
        //procura damas adversárias
        for(let i=0; i<pecas_damas_brancas.length; i++){
          ...
          //procura peças comuns adversárias
          if(andou == 0){
      ...
      if(andou == 0){
        ...
        if(num == 1 || num == 2){
          pecas_damas_brancas = pecas_damas_brancas.reverse();
        }
        for(let i=0; i<pecas_damas_brancas.length; i++){
          branca = pecas_damas_brancas[i].split('');
          switch (num) {
      ...
      //anda com a peça comum
      if(andou == 0){
        num = Math.floor(Math.random() * 10 + 1);
        if(num%2 == 0){
          pecas_comuns_brancas = pecas_comuns_brancas.reverse();
        }
        for(let i=0; i<pecas_comuns_brancas.length; i++){
          branca = pecas_comuns_brancas[i].split('');
          if(num%2 == 0){
      ...
      if(andou == 0){
        num = Math.floor(Math.random() * 4 + 1);
        if(num == 1 || num == 2){
          pecas_damas_pretas = pecas_damas_pretas.reverse();
        } 
        for(let i=0; i<pecas_damas_pretas.length; i++){
          preta = pecas_damas_pretas[i].split('');
          switch (num) {
      ...
      if(andou == 0){
        num = Math.floor(Math.random() * 10 + 1);
        if(num%2 == 0){
          pecas_comuns_pretas = pecas_comuns_pretas.reverse();
        }
        for(let i=0; i<pecas_comuns_pretas.length; i++){
          preta = pecas_comuns_pretas[i].split('');
          if(num%2 == 0){
    ...
    //verifica se o jogo vai ser passo a passo ou automatizado
    if(passo == 0){
...
//função que inicia o jogo e posiciona as peças
...
//função para verificar movimentos possíveis de acordo com o tipo de peça
...
//função para verificar possíveis movimentos
...
//função que verifica todo o processo de movimentação
function seleciona(x,y){
    ...
    if(document.getElementById("jogador_habilitado").innerText == vez){
      if(x>0 && x <9 && y >0 && y <9){
        ...
        if((movimenta['selecionada']['x']==0 || peca[x][y]['cor'] == movimenta['selecionada']['cor']) && peca[x][y]['cor']==vez){
        ...
        }else if(verifica_possivel(x,y,cont_possiveis)){ //se a cor da peca destino for diferente da selecionada
          auxx = x;
          auxy = y;
          auxn = 0;
          mudou = 0;

          //verifica se o segundo local escolhido possui peça de cor diferente
          if(peca[x][y]['cor'] != movimenta['selecionada']['cor']){
            movimenta['destino']['x'] = x;	//recebe o x do destino
            movimenta['destino']['y'] = y;  //recebe y do destino
            ...
            if(auxn==0){
              if(peca[auxx][auxy]['peca'] === false && "t"+movimenta['selecionada']['x']+""+movimenta['selecionada']['y'] != 't00'){
                ...
                //registra o log de alteração da peça
                document.getElementById('log').innerHTML = 'Peça '+movimenta['selecionada']['peca']+' '+movimenta['selecionada']['cor']+' andou para: X = '+auxy+', Y = '+auxx
                ...
                if("t"+movimenta['selecionada']['x']+""+movimenta['selecionada']['y'] != 't00' && x-1>0 && x+1 <9 && y-1>0 && y+1<9){
                  ...
                  //registra o log de alteração da peça
                  document.getElementById('log').innerHTML = 'Peça '+movimenta['selecionada']['peca']+' '+movimenta['selecionada']['cor']+' andou para: X = '+movimenta['selecionada']['x']+', Y = '+auxx
          ...
          //Verifica se foi a peça eliminou outra ou se movimentou
          if(auxn==0){
            if(mudou == 1){
              if(vez=="BRANCO"){
                document.getElementById("ultimo_jogador").innerHTML = vez;
                document.getElementById("passos_brancas").innerHTML = parseInt(document.getElementById("passos_brancas").innerText)+1;
                vez="PRETO";
                document.getElementById("jogador_habilitado").innerHTML = vez;
              }else{
                document.getElementById("ultimo_jogador").innerHTML = vez;
                document.getElementById("passos_pretas").innerHTML = parseInt(document.getElementById("passos_pretas").innerText)+1;
                vez="BRANCO";
                document.getElementById("jogador_habilitado").innerHTML = vez;
              }
            }
          ...
          verifica_pretas = parseInt(document.getElementById("pecas_pretas_removidas").innerText);
